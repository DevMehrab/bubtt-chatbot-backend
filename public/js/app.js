...
const API_BASE = "https://bubtt-chatbot-backend.onrender.com/api";
...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("click", generateWeeklyMealPlan);
  document
    .getElementById("loadSDGBtn")
    .addEventListener("click", loadSDGProfile);
}
...
async function loadConversationHistory() {
  try {
    const response = await fetch(
      `${API_BASE}/user/${currentUserId}/conversations?limit=50`
    );
    const data = await response.json();
    ...
    if (data.success && data.conversations && data.conversations.length > 0) {
      ...
      showToast(
        `Loaded ${data.conversations.length} previous messages`,
        "default"
      );
      console.log(
...
async function loadAnalytics() {
  ...
  try {
    const response = await fetch(
      `${API_BASE}/user/${currentUserId}/sdg-profile`
    );
    const data = await response.json();
    ...
    if (data.success) {
      ...
      let detailsHTML = `
                ...
                <div style="margin-top: 15px;">
                    ...
                    <p><strong>Waste Percentage:</strong> ${
                      data.waste.wastePercentage
                    }%</p>
                    <p><strong>Consumed Items:</strong> ${
                      data.waste.consumedCount
                    }</p>
                    <p><strong>Wasted Items:</strong> ${
                      data.waste.wastedCount
                    }</p>
                </div>
                <div style="margin-top: 15px;">
    ...
    if (data.success) {
      ...
      if (data.inventory.length === 0) {
        inventoryList.innerHTML =
          '<p style="text-align: center; color: #7f8c8d;">No items in inventory</p>';
      } else {
...
async function addInventoryItem() {
  ...
  try {
    const response = await fetch(
      `${API_BASE}/user/${currentUserId}/inventory`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      if (data.mealPlan.meals && data.mealPlan.meals.length > 0) {
        mealHTML += data.mealPlan.meals
          .map(
            (meal) => `
                    <div class="meal-item">
                        <div class="meal-time">🍽️ ${
                          meal.mealType || "Meal"
                        }</div>
                        <div class="meal-recipe"><strong>${
        ...
        mealHTML +=
          "<p>No meals could be generated. Add items to your inventory!</p>";
      }
      mealHTML += "</div>";
...
async function generateWeeklyMealPlan() {
  ...
  try {
    const response = await fetch(
      `${API_BASE}/user/${currentUserId}/weekly-meal-plan`
    );
    const data = await response.json();
    ...
    if (data.success) {
      ...
      if (data.weeklyPlan.mealPlan && data.weeklyPlan.mealPlan.length > 0) {
        ...
        weeklyHTML +=
          "<p>No weekly plan could be generated. Add items to your inventory!</p>";
      }
      weeklyHTML += "</div>";
  ...
  try {
    ...
    if (data.success) {
      ...
      let nutritionHTML = `
                ...
                <p><strong>Variety Score:</strong> ${
                  data.nutrition.varietyScore
                }%</p>
                <p><strong>Suggestions:</strong></p>
                <ul style="margin-left: 20px;">
                    ${data.nutrition.suggestions
                      .map((s) => `<li>${s}</li>`)
                      .join("")}
                </ul>
            `;
